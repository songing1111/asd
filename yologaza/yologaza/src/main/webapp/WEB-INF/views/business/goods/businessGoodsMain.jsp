...
function search_goods_list(fixeSearchPeriod){
    ...
    formObj.method="get";
    formObj.action="${contextPath}/business/goods/businessGoodsMain.do";
    formObj.submit();
}
   ...
   <c:choose>
     ...
     <c:forEach var="item" items="${newGoodsList }" varStatus="i">
			 <TR>       
				...
				<TD >
				 <a href="${pageContext.request.contextPath}/business/goods/modifyGoodsForm.do?goods_id=${item.goods_id}">
				    <strong>${item.goods_name } </strong>
				 </a> 
				</TD>
				<TD>
				<strong>${item.goods_address }</strong> 
				</TD>
				<TD >
				   <strong>${item.goods_checkIn }</strong> 
				</TD>
				<td>
				  <strong>${item.goods_checkIn }</strong>
				</td>
           ...
           <tr>
             <td colspan=8 class="fixed">
		         ...
		         <c:if test="${section >1 && page==1 }">
		          <a href="${contextPath}/business/goods/businessGoodsMain.do?chapter=${section-1}&pageNum=${(section-1)*10 +1 }">&nbsp; prev</a>
		         </c:if>
		          <a href="${contextPath}/business/goods/businessGoodsMain.do?chapter=${section}&pageNum=${page}">${section*10 +page } </a>
		         <c:if test="${page ==10 }">
		          <a href="${contextPath}/business/goods/businessGoodsMain.do?chapter=${section+1}&pageNum=${section*10+1}">&nbsp; next</a>
		         </c:if> 
	      		</c:forEach> 
...
<H3>상품등록하기</H3>
<DIV id="search">
	<form action="${contextPath}/business/goods/addNewGoodsForm.do">
